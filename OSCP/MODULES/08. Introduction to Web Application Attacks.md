...
## Tags
#OSCP #Module8 #WebApplicationAttacks #Nmap #Wappalyzer #Gobuster #BurpSuite #XSS #IDOR #ObjectLevelAuthorization #API #OWASP #Ffuf #WebFuzzing #VHostFuzzing #ZAP #StoredXSS #ReflectedXSS #DOMXSS #BlindXSS #SessionHijacking #Phishing #HTBSupplementary
...
### 8.3.4. WordPress REST disclosure to exposed application artifacts

A WordPress REST response can disclose author identifiers without authentication. Treat the identifier as an enumeration result, then inspect linked application directories. If a plugin browser exposes a JAR, move to offline archive and bytecode analysis instead of trying to exploit WordPress blindly.
...
Route this chain through [[OSCP/RUNBOOK V2/Linux - CMS Check|Linux - CMS Check]], [[OSCP/RUNBOOK V2/Linux - Binary Analysis|Linux - Binary Analysis]], and [[OSCP/RUNBOOK V2/Linux - Credential Search|Linux - Credential Search]]. The complete example is [[OSCP/BOXES/WRITE UPS/Linux/Blocky|Blocky]].

### 8.3.5. IDOR in custom dashboards

Insecure Direct Object Reference is an object-level authorization failure: the application uses a client-supplied identifier but does not verify that the current user may access the referenced object. Custom dashboards may expose numeric records, reports, or packet captures rather than a JSON API.

Compare adjacent IDs and record status, response length, body content, and linked download routes. A different response is a lead; the missing ownership check is the vulnerability. If the object is a PCAP, save it privately and route to [[OSCP/RUNBOOK V2/Linux - IDOR and PCAP Credential Recovery|Linux - IDOR and PCAP Credential Recovery]] for protocol-aware analysis.
...
The complete web-to-evidence chain is [[OSCP/BOXES/WRITE UPS/Linux/Cap|Cap]]. Keep the original capture and any authentication-bearing output out of screenshots, shared logs, and vault notes.

#### Tags: #IDOR #ObjectLevelAuthorization #AccessControl #Module8
...
- **[[OSCP/BOXES/WRITE UPS/Linux/Poison|Poison]]** (HTB, FreeBSD, Easy): LFI via a GET parameter, PHP source review, repeated-encoding credential recovery, and SSH/VNC tunnelling. The external reference documents the alternate log-poisoning route.
...
**IDOR and evidence recovery:**
- **[[OSCP/BOXES/WRITE UPS/Linux/Cap|Cap]]** (HTB, Linux, Easy): predictable dashboard IDs exposed another user's downloadable PCAP; packet analysis recovered FTP authentication and led to controlled SSH validation.

**Application deserialization and configuration pivot:**
- **[[OSCP/BOXES/WRITE UPS/Linux/Management|Management]]** (HTB, Linux): a pre-authentication OpenAM deserialization foothold led to application configuration review and a controlled credential pivot.

#### Tags: #RelatedBoxes #HTBPractice #Module8 #IDOR #ObjectLevelAuthorization
...
- [[OSCP/BOXES/WRITE UPS/Linux/DevOops|DevOops]] -- multipart XML upload, source-first XXE, and Python deserialization proof
- [[OSCP/BOXES/WRITE UPS/Linux/Mirai|Mirai]] -- product-header fingerprinting, `/admin/` discovery, and the decision to validate an IoT account through SSH
- [[OSCP/BOXES/WRITE UPS/Linux/Blocky|Blocky]] -- WordPress REST disclosure, exposed plugin artifact review, and slow-response timing triage
