...
### Blocky: WordPress disclosure and Java plugin artifact review

Blocky is a useful application-chain example even though WordPress itself is not exploited. Fingerprint WordPress, query its REST users endpoint, inspect the exposed plugin browser, and treat the JAR as a local evidence source. The Java class contained a reusable credential, which routed to SSH and then Linux sudo.

- Public REST data identifies an account, not a password.
- A plugin browser can expose a custom artifact even when CMS exploitation is unnecessary.
- jar tf and javap belong in the offline analysis lane.
- Slow REST responses should be measured before changing the VPN.

See [[OSCP/BOXES/WRITE UPS/Linux/Blocky|Blocky]], [[OSCP/RUNBOOK V2/Linux - CMS Check|Linux - CMS Check]], and [[OSCP/RUNBOOK V2/Linux - Binary Analysis|Linux - Binary Analysis]].

### Shocker worked example

[[OSCP/BOXES/WRITE UPS/Linux/Shocker|Shocker]] applies the Shellshock concept above as a complete route. A forbidden CGI directory was enumerated directly, the User-Agent proof used id before the callback, and the landed shelly shell routed to the exact passwordless Perl rule in local sudoers.

### Cap worked example: IDOR to packet-analysis credential recovery

[[OSCP/BOXES/WRITE UPS/Linux/Cap|Cap]] applies the IDOR concept to a custom dashboard rather than a profile API. Comparing predictable data IDs revealed a downloadable PCAP without an ownership check. capinfos and tshark then turned the web-disclosed artifact into an FTP credential hypothesis, which was validated once over SSH. The lesson is to follow the object reference into its linked artifact and preserve the evidence chain; a 200 response or a live anonymous-FTP denial is not enough to close the branch.

See [[OSCP/RUNBOOK V2/Linux - IDOR and PCAP Credential Recovery|Linux - IDOR and PCAP Credential Recovery]] and [[OSCP/MODULES/16. Password Attacks#16.3.6.4. Network Traffic Credential Capture (Wireshark)|16.3.6.4]].

### Busqueda worked example: Searchor Python eval command injection

Busqueda shows why application fingerprinting and source review matter when the exploit database does not provide a ready-made route. The application identified Searchor 2.4.0 and built a Python expression equivalent to:

```python
eval(f"Engine.{engine}.search('{query}')")
```

The `query` value was inserted inside a single-quoted Python string. Closing that string, closing the original function call, adding an `os.popen()` expression, and commenting the remaining characters produced reflected command output as the service account.
...
The payload has two interpreters in sequence. Python `eval()` handles the first layer. The `os.popen()` call then launches the operating-system command. This distinction explains why a shell operator alone is not the first test and why URL encoding is required for the final request.

Use `id` as the first proof because it confirms the Python expression, the imported module, the command, and the response channel without introducing callback quoting. If the response contains the service identity, deliver the callback through the same expression and move to [[OSCP/RUNBOOK V2/Linux - RCE to Shell|Linux - RCE to Shell]].

> [!warning] Gotcha
> Searchor payloads fail easily when shell quoting, Python quoting, form parsing, and URL encoding are mixed together. Preserve a baseline response, send one field at a time with `--data-urlencode`, and keep reflected execution proof separate from callback proof.

> [!tip] Efficiency
> A public exploit search with no useful result is not a reason to abandon a versioned application. Read the source or deployed package, identify the expression boundary, and construct the smallest harmless proof from the actual data flow.

See [[OSCP/BOXES/WRITE UPS/Linux/Busqueda|Busqueda]] and [[OSCP/RUNBOOK V2/Linux - Command Injection|Linux - Command Injection]].

#### Tags: #Searchor #PythonEval #CommandInjection #ReflectedRCE #Module9
...
- **[[OSCP/BOXES/WRITE UPS/Linux/Poison|Poison]]** (HTB, FreeBSD, Easy): LFI through a GET parameter, PHP source disclosure with `php://filter`, mechanical decoding of an exposed credential backup, and SSH/VNC tunnelling. The external reference documents an alternate log-poisoning route, but this completed run used the credential and loopback-service path.
...
- **[[Snookums|Snookums]]** (PG Practice, Linux, Medium — done ✅): LFI via unsanitised `include()` in Simple PHP Photo Gallery. Discover parameter with ffuf, read PHP source via `php://filter`, execute PHP via `data://` wrapper (no outbound network needed). SELinux `httpd_t` blocks reverse/bind shells — work entirely through the web channel. Uses [[OSCP/RUNBOOK V2/Linux - LFI|Web App - LFI]] and [[OSCP/RUNBOOK V2/Linux - RFI|Web App - RFI]] stage notes.
...
- **[[OSCP/BOXES/WRITE UPS/Linux/Traceback|Traceback]]** (HTB, Linux, Easy): homepage source comment disclosed an attacker-left PHP shell; targeted filename enumeration found SmEvK, whose default credentials exposed a command console as `webadmin`. Core lesson: treat attacker clues and exposed development artefacts as high-priority web enumeration leads.

**IDOR / HTTP Verb Tampering / XXE:**
- **[[OSCP/BOXES/WRITE UPS/Linux/Cap|Cap]]** (HTB, Linux, Easy): dashboard IDOR -> downloadable PCAP -> tshark FTP credential recovery -> SSH reuse. Core lesson: compare object responses and follow the artifact; preserve cleartext authentication privately and validate it once.
- **[[OSCP/BOXES/WRITE UPS/Linux/DevOops|DevOops]]** (HTB, Linux, Medium): multipart XML upload -> reflected XXE file read -> Flask source disclosure -> unsafe Python pickle command proof -> SSH key. Core lesson: source-first web enumeration and Git-history credential hunting.
- **[[OSCP/BOXES/WRITE UPS/Linux/Mirai|Mirai]]** (HTB, Linux, Easy): Pi-hole product-header fingerprinting -> `/admin/` discovery -> controlled IoT credential validation over SSH. Core lesson: a 404 can still fingerprint a product, and an old appliance should be tested through an evidence-based account path before exploit hunting.
...
- [[OSCP/BOXES/WRITE UPS/Windows/Buff|Buff]] -- unauthenticated Gym Management System 1.0 upload RCE and web-shell foothold
- [[OSCP/BOXES/WRITE UPS/Linux/Blocky|Blocky]] -- WordPress REST disclosure, plugin JAR analysis, and credential reuse routing to SSH
- [[OSCP/BOXES/WRITE UPS/Linux/Management|Management]] -- pre-authentication OpenAM deserialization and a web-to-local application-secret pivot
- [[OSCP/BOXES/WRITE UPS/Linux/Busqueda|Busqueda]] -- Searchor Python eval injection, source-driven input analysis, and a web-to-local credential pivot
- [[OSCP/BOXES/WRITE UPS/Linux/Nukem|Nukem]] -- WordPress plugin upload handling followed by webshell verification
- [[OSCP/BOXES/WRITE UPS/Linux/Snookums|Snookums]] -- LFI/RFI and data-wrapper credential recovery before local escalation
- [[OSCP/BOXES/WRITE UPS/Linux/Networked|Networked]] -- upload-filter bypass with a predictable client-IP path
- [[OSCP/BOXES/WRITE UPS/Linux/Sea|Sea]] -- web application identification and command-injection attack chain
- [[OSCP/BOXES/WRITE UPS/Linux/Payday|Payday]] -- CS-Cart LFI, username discovery, and controlled SSH validation
- [[OSCP/BOXES/WRITE UPS/Linux/Pelican|Pelican]] -- unauthenticated Exhibitor configuration command injection to a shell
- [[OSCP/BOXES/WRITE UPS/Linux/SolidState|SolidState]] -- Apache James RMA/POP3 application enumeration and credential-driven shell transition
- [[OSCP/BOXES/WRITE UPS/Linux/SwagShop|SwagShop]] -- Magento fingerprinting, Shoplift SQLi, and authenticated object-injection RCE
- [[OSCP/BOXES/WRITE UPS/Windows/Netmon|Netmon]] -- PRTG application identification, configuration-backup recovery, and notification command injection
