...
- **[[Zenphoto|Zenphoto]]** (PG Practice, Linux, done ✅): Ubuntu 10.04, kernel 2.6.32-21-generic. SUID nothing exploitable, sudo requires password. Kernel LPE via CVE-2010-3904 / EDB-15285 (RDS Protocol). Compiled on target with gcc. Classic old-kernel-as-last-resort path. Technique: [[PrivEsc Linux - Kernel]].
- **[[Nibbles|Nibbles]]** (PG Practice, Linux, done ✅): SUID `/usr/bin/find` - `find . -exec /bin/bash -p \; -quit` for euid=0. Textbook SUID find exploit from [[18. Linux Privilege Escalation#SUID|18.x SUID section]]. Technique: [[PrivEsc Linux - SUID]].
...
- **[[Pebbles|Pebbles]]** (PG Practice, Linux, done ✅): MySQL root creds in `/etc/zm/zm.conf`. MySQL running as OS root. `lib_mysqludf_sys.so` UDF - `sys_exec()` used to create SUID bash at `/tmp/rootbash`. Classic MySQL-as-root privilege escalation pattern. Technique: [[PrivEsc Linux - UDF]].
