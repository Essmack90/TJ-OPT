...
#### 18.2.1.3 Compiled application credentials

A downloaded JAR or other application artifact is also a credential-hunting surface. Preserve the original, list its contents, inspect the relevant class, and search the saved output for connection details before testing a candidate against another service.

~~~bash
file "$BoxDir/loot/$File"
jar tf "$BoxDir/loot/$File" | tee "$BoxDir/loot/jar-contents.txt"
...
The complete web-to-SSH example is [[OSCP/BOXES/WRITE UPS/Linux/Blocky|Blocky]]. Route the artifact review through [[OSCP/RUNBOOK V2/Linux - Binary Analysis|Linux - Binary Analysis]] and [[OSCP/RUNBOOK V2/Linux - Credential Search|Linux - Credential Search]], then validate one likely credential before local enumeration.
...
**HTB Cap case study — versioned interpreter:** local sudo and SUID checks were not the route. getcap identified a versioned Python interpreter with CAP_SETUID, so the exact path was used for os.setuid(0), followed by id and whoami proof. This is a useful reminder that a capability may be attached to a specific interpreter binary rather than the generic command name.
...
> 🔗 **See also:** [[OSCP/RUNBOOK V2/Linux - Sudo Check|PrivEsc Linux - Sudo]] runbook stage note for the decision table and full technique reference.
...
#### 18.4.2.1 Full all-command sudo after password reuse

When a recovered application credential is accepted by SSH, immediately run identity and sudo checks. (ALL : ALL) ALL means the user may run any command as any user, subject to password authentication. Validate the boundary with sudo -l, then use sudo -i and record id and whoami. This is the direct route demonstrated by Blocky; lxd membership was a secondary lead, not the chosen path.
...
### 18.5.12 Busqueda: Docker environment and relative-path sudo execution

Busqueda adds two local-enumeration branches that are easy to miss when a standard SUID and capability review is empty. First, `sudo -l` exposed a custom Python wrapper with a wildcard argument. Its safe-looking Docker actions could inspect environment arrays from containers bound to loopback. Second, the same wrapper's `full-checkup` action invoked `./full-checkup.sh` without anchoring the path to the administrator repository.

Enumerate and read custom sudo scripts before using them:

```bash
sudo -n -l
sed -n '1,260p' "$SudoScript"
grep -nE 'docker|subprocess|full-checkup|arg_list|action' "$SudoScript"
```

The Docker branch is an evidence-preserving way to review the hidden local service configuration:
...
The format string is Docker's Go template expression. The wrapper expects the format string and the container name as separate arguments. Treat environment output as credential material even when the container port is only reachable through `127.0.0.1`.

The winning escalation came from source review of the `full-checkup` path:
...
The relative path is resolved from the child process current working directory, which is inherited from the caller. A controlled helper in the writable `svc` home directory therefore executes when the sudo command is launched from that directory:
...
For an interactive lab proof, replace the marker with a callback and start the listener first. The key condition remains the same: change into the directory containing the controlled helper before invoking sudo. Running the trigger from `/tmp` while the helper is in the home directory is a failure caused by cwd, not by sudo or Python.

> [!warning] Gotcha
> `./full-checkup.sh` is relative to the caller's cwd, not to the directory containing `system-checkup.py` and not to the Gitea checkout. Always inspect the script, create the helper in a controlled writable directory, run `pwd`, and then trigger the sudo action.

> [!tip] Efficiency
> `docker-ps` followed by `docker-inspect '{{json .Config.Env}}' $ContainerName` is a fast way to turn an allowed wrapper into configuration evidence. Do not spend time attacking a loopback service from Kali before checking whether the local shell can inspect its container metadata directly.

See [[OSCP/BOXES/WRITE UPS/Linux/Busqueda|Busqueda]], [[OSCP/RUNBOOK V2/Linux - Docker Enumeration|Linux - Docker Enumeration]], and [[OSCP/RUNBOOK V2/Linux - Sudo Check|Linux - Sudo Check]].

#### Tags: #Docker #ContainerEnumeration #RelativePath #Sudo #Python #Module18
...
- **[[OSCP/BOXES/WRITE UPS/Linux/Knife|Knife]]** -- passwordless `sudo knife` Ruby execution and shell replacement
- **[[OSCP/BOXES/WRITE UPS/Linux/Networked|Networked]]** -- writable cron filename injection followed by sudo configuration parsing
- **[[OSCP/BOXES/WRITE UPS/Linux/Nibbles|Nibbles]]** -- SUID `find` used to preserve an effective UID 0 shell
- **[[OSCP/BOXES/WRITE UPS/Linux/TartarSauce|TartarSauce]]** -- sudo tar checkpoint execution and systemd archive replacement
- **[[OSCP/BOXES/WRITE UPS/Linux/Traverxec|Traverxec]]** -- journalctl sudo shell escape after SSH credential recovery
- **[[OSCP/BOXES/WRITE UPS/Linux/Valentine|Valentine]]** -- readable root tmux socket/session hijack
- **[[Snookums|Snookums]]** (PG Practice, Linux, Medium — done ✅): `/etc/passwd` **owned by the web app user** (not world-writable). Append a UID-0 entry with a known hash, `su` to root. Snookums is the cleaner example than a world-writable /etc/passwd — the distinction between owner-write and world-write matters. Uses [[OSCP/RUNBOOK V2/Linux - Local Enum|PrivEsc Linux - Writable Passwd]] stage note.
- **HTB CronOS** -- classic cron job abuse: web foothold, then a writable root-owned cron task; direct repetition of 18.3.1. See [[OSCP/BOXES/WRITE UPS/Linux/CronOS|CronOS]].
...
- **[[OSCP/BOXES/WRITE UPS/Linux/Shocker|Shocker]]** -- Shellshock CGI foothold, then exact passwordless sudo Perl interpreter escape; covers 18.4.2 sudo and the principle of checking the exact approved executable before slower branches
...
- **[[OSCP/BOXES/WRITE UPS/Linux/Traceback|Traceback]]** -- exposed SmEvK shell → passwordless sudo to Luvit → Lua `os.execute()` as `sysadmin` → group-writable root-run MOTD script → SSH-triggered SUID Bash. Covers interpreter abuse, group/file permission reasoning, login-triggered execution, and the SUID Bash `-p` requirement (18.1.2, 18.4.1, 18.4.2).
...
- **[[OSCP/BOXES/WRITE UPS/Linux/DevOops|DevOops]]** -- source-driven SSH-key recovery followed by Git-history credential hunting; the root path was credential validation rather than a sudo or kernel exploit
- **[[OSCP/BOXES/WRITE UPS/Linux/Mirai|Mirai]]** -- unchanged IoT account validation led to SSH as `pi`, `sudo -n -l` showed unrestricted `NOPASSWD: ALL`, and mounted USB metadata was recorded without reading private completion data
- **[[OSCP/BOXES/WRITE UPS/Linux/Blocky|Blocky]]** -- a Java plugin exposed a reusable credential, SSH confirmed the account, and unrestricted sudo provided the direct escalation route

- **[[Zenphoto|Zenphoto]]** (PG Practice, Linux, done ✅): Ubuntu 10.04, kernel 2.6.32-21-generic. SUID nothing exploitable, sudo requires password. Kernel LPE via CVE-2010-3904 / EDB-15285 (RDS Protocol). Compiled on target with gcc. Classic old-kernel-as-last-resort path. Technique: [[OSCP/RUNBOOK V2/Linux - Kernel Exploit|PrivEsc Linux - Kernel]].
- **[[Nibbles|Nibbles]]** (PG Practice, Linux, done ✅): SUID `/usr/bin/find` - `find . -exec /bin/bash -p \; -quit` for euid=0. Textbook SUID find exploit from [[18. Linux Privilege Escalation#SUID|18.x SUID section]]. Technique: [[OSCP/RUNBOOK V2/Linux - SUID Check|PrivEsc Linux - SUID]].
- **[[Nukem|Nukem]]** (PG Practice, Linux, done ✅): SUID `/usr/bin/dosbox` — not in GTFOBins, requires lateral thinking. DOSBox is a DOS emulator; `-c 'mount c /etc'` maps `/etc` as DOS C: drive, then `-c 'echo USER ALL=(ALL) NOPASSWD: ALL > c:\sudoers'` writes to `/etc/sudoers` as root. `sudo -n bash` → root. Key insight: any SUID-root binary that can write files can write to `/etc/sudoers`. DOSBox's DOS `echo >` is the mechanism. Cleanup requires restoring sudoers from the pacman package archive. Technique: [[OSCP/RUNBOOK V2/Linux - SUID Check|PrivEsc Linux - SUID]].
- **[[Pebbles|Pebbles]]** (PG Practice, Linux, done ✅): MySQL root creds in `/etc/zm/zm.conf`. MySQL running as OS root. `lib_mysqludf_sys.so` UDF - `sys_exec()` used to create SUID bash at `/tmp/rootbash`. Classic MySQL-as-root privilege escalation pattern. Technique: [[OSCP/RUNBOOK V2/Linux - Database Access|PrivEsc Linux - UDF]].
- **[[Cockpit|Cockpit]]** (PG Practice, Linux, done ✅): **sudo tar wildcard injection.** Rule: `(ALL) NOPASSWD: /usr/bin/tar -czvf /tmp/backup.tar.gz *`. Created `--checkpoint=1` and `--checkpoint-action=exec=bash privesc.sh` as filenames in CWD — tar treated them as flags and ran the script as root → SUID bash → root shell. Key gotcha: `exec=scriptname` fails without `bash` prefix (executor doesn't search CWD); `exec=bash scriptname` works. Technique: [[OSCP/RUNBOOK V2/Linux - Sudo Check|PrivEsc Linux - Tar Wildcard]].
...
- **[[OSCP/BOXES/WRITE UPS/Linux/Cap|Cap]]** (HTB, Linux, Easy): no useful sudo/SUID route; a versioned Python interpreter exposed CAP_SETUID and direct UID proof yielded root. See [[OSCP/RUNBOOK V2/Linux - File Capabilities|Linux - File Capabilities]].
- **[[OSCP/BOXES/WRITE UPS/Linux/Management|Management]]** (HTB, Linux): exact sudo-rule review found duplicate rdiff-backup restriction injection; the read-only protocol provided root evidence without a writable target payload. See [[OSCP/RUNBOOK V2/Linux - Rdiff-Backup Sudo Abuse|Linux - Rdiff-Backup Sudo Abuse]].
- **[[OSCP/BOXES/WRITE UPS/Linux/Jarvis|Jarvis]]** -- SUID `systemctl` editor execution created a SUID Bash helper.
- **[[OSCP/BOXES/WRITE UPS/Linux/Payday|Payday]]** -- unrestricted sudo was validated as the direct root boundary.
- **[[OSCP/BOXES/WRITE UPS/Linux/Pelican|Pelican]]** -- sudo `gcore` exposed a root-process credential in memory.
- **[[OSCP/BOXES/WRITE UPS/Linux/SolidState|SolidState]]** -- restricted-shell behavior was assessed before returning to credential and authenticated file-write branches.
- **[[OSCP/BOXES/WRITE UPS/Linux/SwagShop|SwagShop]]** -- exact passwordless Vim execution became the root shell boundary.

#### Tags: #RelatedBoxes #HTBCronOS #HTBNibbles #HTBShocker #HTBTraverxec #HTBOpenAdmin #HTBValentine #HTBCap #LinuxPrivesc #Module18
...
- [[OSCP/RUNBOOK V2/Linux - IoT Default Credentials]]
- [[OSCP/RUNBOOK V2/Linux - Sudo Check]]
- [[OSCP/RUNBOOK V2/Linux - File Capabilities]]
...
- [[OSCP/BOXES/WRITE UPS/Linux/Bashed|Bashed]] -- writable cron execution and SUID Bash verification
- [[OSCP/BOXES/WRITE UPS/Linux/Cockpit|Cockpit]] -- tar wildcard checkpoint-action abuse
- [[OSCP/BOXES/WRITE UPS/Linux/Nukem|Nukem]] -- SUID DOSBox to controlled sudoers modification
- [[OSCP/BOXES/WRITE UPS/Linux/Snookums|Snookums]] -- owner-writable `/etc/passwd` escalation
