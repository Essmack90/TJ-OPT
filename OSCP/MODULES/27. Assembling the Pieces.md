...
🔁 Similar to: [[26. Attacking AWS Cloud Infrastructure#26.4.2 Hunting Credentials in Git History|Attacking AWS Cloud Infrastructure#26.4.2 Hunting Credentials in Git History]], [[OSCP/MODULES/26. Attacking AWS Cloud Infrastructure#26.4.2 Hunting Credentials in Git History|GitHappens]] (HTB)
...
- ippsec.rocks → search "ntlm relay" or "responder relay" — [[OSCP/BOXES/MASTER BOX LIST|Responder]] (HTB), MultiMaster (HTB) both cover this chain
...
## Busqueda cross-box chain: small findings joined into root

Busqueda is a Linux-scale version of the capstone principle. No single finding was enough. The web version identified a Python expression boundary, the Git configuration supplied a credential candidate, SSH supplied a stable local shell, Docker inspection supplied a second credential scope, and source review exposed the final cwd-controlled sudo execution.

```mermaid
flowchart LR
    A["Searchor 2.4.0\nPython eval()"] --> B["Reflected id\nsvc context"]
...
The reusable capstone habit is to record what each result changes:

| Result | New decision |
|---|---|
| Searchor response contains the service identity | The Python expression is executable, so callback delivery is a separate problem |
| Git remote contains HTTP credentials | Test the value against the service and accounts suggested by the evidence |
| SSH as `svc` succeeds | Stop treating the web callback as the only shell and run native local enumeration |
| Docker environment is readable through sudo | Inspect local service configuration before attacking exposed ports blindly |
| Gitea administrator source is readable | Review the invoked helper and its path handling |
| `./full-checkup.sh` is relative | Set the cwd deliberately, create a controlled helper, and validate root execution |

This chain also reinforces the documentation discipline used by the Challenge Labs. Keep the source artifact, command output, credential source, validation result, and final proof linked in the case workspace. The vault should contain the method and routing, while private loot retains values and flags.

See [[OSCP/BOXES/WRITE UPS/Linux/Busqueda|Busqueda]], [[OSCP/MODULES/09. Common Web Application Attacks|Module 9]], [[OSCP/MODULES/16. Password Attacks|Module 16]], and [[OSCP/MODULES/18. Linux Privilege Escalation|Module 18]].

#### Tags: #HTBCapstone #Busqueda #CommandInjection #CredentialReuse #Docker #Sudo #RelativePath #Module27
...
- [[OSCP/BOXES/MASTER BOX LIST|Outdated]] (HTB Windows, Easy) — phishing with a Windows Library file + WebDAV shortcut → internal reverse shell; direct technique match for 27.3.2. The most 1:1 public box for the phishing delivery chain.
...
- [[OSCP/BOXES/MASTER BOX LIST|Intelligence]] (HTB Windows, Medium) — multi-stage AD chain: credential hunting in PDFs → password spray → constrained delegation → DCSync; mirrors the "chain of small wins" structure throughout 27.4-27.6. Good practice for accepting that no single step gives you everything.
...
- [[OSCP/BOXES/MASTER BOX LIST|Monteverde]] (HTB Windows, Medium) — Azure AD Connect service account → credential extraction → DCSync; good practice for the DCSync final step (27.6.3) in a different escalation context.
- [[OSCP/BOXES/MASTER BOX LIST|Resourced]] (PG Practice, Intermediate) — full AD chain ending in DCSync via resource-based constrained delegation; one of the better PG boxes for practicing the complete credential dump → domain admin sequence.
...
- [[OSCP/BOXES/MASTER BOX LIST|Support]] (HTB Windows, Easy) — BloodHound-driven attack chain; different initial vector but the "who has a session where, and what can they reach" BloodHound analysis in 27.4 is essentially identical.
- [[OSCP/BOXES/MASTER BOX LIST|Cascade]] (HTB Windows, Medium) — deep AD enumeration (legacy LDAP, VNC password, AD Recycle Bin) → credential cascade; good for practising methodical "what did I miss in enumeration" thinking from 27.1-27.4.
- [[OSCP/BOXES/MASTER BOX LIST|Resolute]] (HTB Windows, Medium) — password spray → evil-winrm → DLL hijack → DNSAdmin group → DC shell; different privilege escalation path but the spray-and-pivot methodology overlaps with 27.3.1.
...
- [[OSCP/BOXES/WRITE UPS/Linux/Busqueda|Busqueda]] -- reflected RCE, Git and Docker credential pivots, Gitea validation, and relative-path sudo execution
