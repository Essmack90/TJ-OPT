...
## Vintage application: NTLM boundary, targeted Kerberoasting, and DPAPI handoff

Vintage separates authentication validity from client support. `P.Rosa` obtained a Kerberos TGT while NTLM returned `STATUS_NOT_SUPPORTED`. Later, the gMSA cache requested a targeted `svc_sql` ticket after ACL-controlled SPN creation. The installed Impacket version's automatic enumeration path failed, so the exact account was supplied with `-usersfile`.

The ticket crack was then validated as password reuse on `C.Neri`. That foothold led to Windows Credential Manager and DPAPI rather than a direct token escalation. `dpapi.py masterkey` required the exact SID and password, and `dpapi.py credential` required the correct masterkey mapping with the `0x` prefix preserved. See [[OSCP/BOXES/WRITE UPS/AD/Vintage|Vintage]], [[OSCP/RUNBOOK V2/AD - Kerberoasting|AD - Kerberoasting]], and [[OSCP/RUNBOOK V2/AD - Local Credential Search|AD - Local Credential Search]].

## Search application: TGS, PFX, and certificate authentication

Search joins two offline cracking branches: a Kerberos TGS for web_svc and a PKCS#12 PFX from a redirected user profile. The PFX then authenticates Sierra to IIS and exposes PSWA. The key distinction is that the certificate authenticates to the web service; the resulting Windows session still needs normal identity and group triage.
...
- [[OSCP/BOXES/WRITE UPS/AD/Vintage|Vintage]] -- Kerberos-only validation, gMSA-controlled SPN, targeted Kerberoast, password reuse, and DPAPI credential recovery
- [[OSCP/BOXES/WRITE UPS/AD/Blackfield|Blackfield]] -- AS-REP roasting, offline cracking, and administrator pass-the-hash after NTDS extraction
- [[OSCP/BOXES/WRITE UPS/AD/Active|Active]] -- GPP `cpassword` recovery, Kerberoasting, and password spraying/reuse validation
- [[OSCP/BOXES/WRITE UPS/AD/Return|Return]] -- LDAP passback used to capture a service credential for authenticated access
- [[OSCP/BOXES/WRITE UPS/AD/Fermion|Fermion]] -- Winlogon credential validation followed by authenticated SMB access and pass-the-hash proof
- [[OSCP/BOXES/WRITE UPS/AD/RockyColt|RockyColt]] -- NTLM/ccache authentication and delegated RBCD validation
## Why this matters for OSCP

This page matters because it turns a repeatable assessment task into a clear, reviewable habit for the OSCP exam.

## Escape case study: ESC1 certificate-template abuse

Escape provides a complete ESC1 path from a normal domain user to an Administrator-authentication certificate. The route depends on configuration, not on the template name alone.

### Confirm the vulnerable template

Run Certipy with the domain user credential and save the detailed output privately:
...
The decision requires all three conditions:

1. The enrollee can supply the subject or UPN.
2. The template includes client authentication or another authentication-capable EKU.
3. The current user or one of their groups has enrollment rights.

If one condition is absent, do not treat the template as a working ESC1 path. Route to the next authentication or privilege branch.

### Request and authenticate the certificate
...
The certificate request and certificate authentication are separate checkpoints. A valid PFX can still fail at authentication when the local clock is outside the Kerberos tolerance. On Certipy v5, redirect output rather than copying older `-output` syntax without checking local help.

The successful certificate authentication returned the Administrator NT hash privately. That hash was then consumed by the pass-the-hash route in Module 24. A certificate, PFX passphrase, password, Net-NTLMv2 response, and NT hash are different credential types with different validation paths.

See [[OSCP/BOXES/WRITE UPS/Windows/Escape|Escape]], [[OSCP/RUNBOOK V2/AD - Certificate Services ESC1|AD Certificate Services ESC1]], and [[OSCP/RUNBOOK V2/AD - Clock Sync|AD Clock Sync]].

## Kerberos clock skew in practice

Measure the DC offset before running ticket or certificate operations:

```bash
ntpdig -p 1 "$BoxIP"
```

If the target remains offset, wrap the one command that needs Kerberos time:
...
Changing the workstation clock may disrupt VPN connectivity and other evidence collection. A process-scoped clock is easier to reproduce and easier to clean up.

## Related box coverage

- [[OSCP/BOXES/WRITE UPS/Windows/Escape|Escape]] -- ESC1 template validation, UPN impersonation, Certipy v5 clock handling, and certificate-to-hash authentication
