...
- **[[Nibbles|Nibbles]]** (PG Practice, Linux, done ✅): PostgreSQL 11.3 with default creds. `COPY TO PROGRAM` used for OS command execution — the SQL-to-RCE pattern from [[10. SQL Injection Attacks#10.3. Manual and Automated Code Execution|10.3]], applied directly via psql rather than through a web app SQLi. Key lesson: COPY TO PROGRAM uses `/bin/sh` not bash — `>&` redirect fails on Debian (dash). Use mkfifo+nc. Technique: [[OSCP/RUNBOOK V2/Linux - Database Access|PostgreSQL - COPY TO PROGRAM RCE]].
- **[[OSCP/BOXES/WRITE UPS/Linux/Jarvis|Jarvis]]** -- numeric SQLi, manual UNION extraction, MariaDB `INTO OUTFILE` webshell delivery, and a source-reviewed post-SQL command path.
- **[[OSCP/BOXES/WRITE UPS/Linux/SwagShop|SwagShop]]** -- Magento Shoplift SQLi used to create a controlled administrative account before authenticated object-injection RCE.
- **[[Pebbles|Pebbles]]** (PG Practice, Linux, done ✅): ZoneMinder 1.29.0 — SQLi in the `limit` POST parameter (numeric, stacked queries, no quoting). SLEEP test confirmed timing. Web root leaked from verbose SQL error in the JSON response. INTO OUTFILE webshell → www-data RCE → reverse shell. Directly matches [[10. SQL Injection Attacks#10.3. Manual and Automated Code Execution|10.3 webshell via MySQL INTO OUTFILE]]. Technique: [[OSCP/RUNBOOK V2/Linux - SQLi|Foothold - SQLi to Shell]], [[OSCP/RUNBOOK V2/Linux - SQLi|Web App - SQLi]].
- **[[Cockpit|Cockpit]]** (PG Practice, Linux, done ✅): **SQLi auth bypass with WAF keyword bypass.** Login form blocked `OR` keyword — `' || 1=1#` using MySQL's `||` boolean operator bypassed the filter. Dashboard dumped base64-encoded OS creds for two users. Key lesson: when a WAF blocks `OR`, try `||` — same MySQL boolean result, different string. Technique: [[OSCP/RUNBOOK V2/Linux - SQLi|Web App - SQLi]].
...
- [[OSCP/RUNBOOK V2/Linux - SQLi]]
- [[OSCP/RUNBOOK V2/Linux - Database Access]]
...
- [[OSCP/BOXES/WRITE UPS/Linux/Cockpit|Cockpit]] -- SQLi WAF bypass and credential recovery
- [[OSCP/BOXES/WRITE UPS/Linux/Nibbles|Nibbles]] -- PostgreSQL `COPY TO PROGRAM` after validating database access
