...
### 6.6.8. Slow application responses and cross-service clues

A slow dynamic endpoint is not automatically a broken VPN. Compare Nmap reachability, a known static path, and curl timing before changing the route or restarting OpenVPN.
...
If the host exposes HTTP, SSH, FTP, and a second application service such as Minecraft, keep the full service set in scope. A web-exposed plugin or client artifact may be the bridge to credentials used by SSH. See [[OSCP/BOXES/WRITE UPS/Linux/Blocky|Blocky]], [[OSCP/RUNBOOK V2/Linux - Web Enum|Linux - Web Enum]], and [[OSCP/RUNBOOK V2/Linux - Service Scan|Linux - Service Scan]].

### 6.6.9. Application objects and downloadable evidence

Custom dashboards may expose records through predictable IDs even when the page has no obvious API. Compare adjacent objects, preserve response metadata, and follow a discovered download route only after recording the access-control difference.
...
If the artifact is a PCAP, use the protocol tree to choose tshark filters. FTP credentials inside the capture can be useful even when the live FTP service rejects anonymous access. Store the PCAP and extracted fields privately, validate the recovered credential once, and continue with the resulting service. See [[OSCP/RUNBOOK V2/Linux - IDOR and PCAP Credential Recovery|Linux - IDOR and PCAP Credential Recovery]] and [[OSCP/BOXES/WRITE UPS/Linux/Cap|Cap]].

> 🔍 **Worth remembering generally:** a 200 status is not enough to establish IDOR. An empty record and another user's record may share the same status, so compare body length and content.
...
- **[[Bratarina|Bratarina]]** (PG Practice, Linux, Easy): SMTP banner grab (`nc $BoxIP 25`) reveals OpenSMTPD 6.6.2, directly enabling the CVE-2020-7247 exploit path. SMB null session secondary finding (passwd.bak). Practical [[06. Information Gathering#6.4.5. SMTP Enumeration|6.4.5]] + [[06. Information Gathering#6.4.4. SMB Enumeration|6.4.4]] recon chain, though the foothold is a public exploit rather than recon itself.
...
- **[[Zenphoto|Zenphoto]]** (PG Practice, Linux, done ✅): Root of port 80 returned a blank placeholder page. Gobuster with `common.txt` found `/test/` — the entire attack surface. Without dir busting, nothing would have been found. Practical [[06. Information Gathering#6.5. Web Application Enumeration|web enumeration]] example. Technique: [[OSCP/RUNBOOK V2/Linux - Web Enum|HTTP - Directory Brute]].
- **[[Nibbles|Nibbles]]** (PG Practice, Linux, done ✅): PostgreSQL 11.3 found on non-standard port 5437 via `-sV` service scan (nmap shows it as `pmip6-data` by port number alone). Default creds `postgres:postgres` on first try. Practical example of why service scanning matters over port-number-only triage.
- **[[OSCP/BOXES/WRITE UPS/Linux/Mirai|Mirai]]** (HTB, Linux, Easy): full TCP scanning found SSH, DNS, lighttpd, Plex, and UPnP services; a 404 response still disclosed Pi-hole through its header, and `/admin/` supplied the product version. Practical example of complete port coverage and product-aware web fingerprinting.
...
- **[[OSCP/BOXES/WRITE UPS/Linux/Cap|Cap]]** (HTB, Linux, Easy): full service enumeration found FTP, SSH, and a Gunicorn dashboard; a bounded object-ID comparison exposed a PCAP whose FTP authentication became the foothold. Practical example of joining web access-control testing with offline protocol analysis.
- **[[OSCP/BOXES/WRITE UPS/Linux/Management|Management]]** (HTB, Linux): full-port and service enumeration routed from mixed Nginx/OpenAM/RMI/OpenDJ services into application-aware credential hunting.
- **[[OSCP/BOXES/WRITE UPS/Windows/Conceal|Conceal]]** (HTB, Windows): a filtered TCP result was not treated as host failure; UDP SNMP/IKE discovery exposed the authenticated transport path.
- **[[OSCP/BOXES/WRITE UPS/Windows/Netmon|Netmon]]** (HTB, Windows): anonymous FTP was recognized as Windows filesystem access and routed into ProgramData/PRTG configuration review.
- **[[OSCP/BOXES/WRITE UPS/Windows/Grandpa|Grandpa]]** (HTB, Windows, Easy): IIS 6.0/WebDAV identification came from complete port and service enumeration, then the legacy banner/method evidence was mapped to CVE-2017-7269. The manual callback failure was retained as evidence rather than treated as a dead end; see [[OSCP/RUNBOOK V2/Windows - Service Scan|Windows - Service Scan]], [[OSCP/RUNBOOK V2/Windows - Web Enum|Windows - Web Enum]], and [[OSCP/RUNBOOK V2/Windows - Exploit Search|Windows - Exploit Search]].
...
- [[OSCP/BOXES/WRITE UPS/Linux/Blocky|Blocky]] -- cross-service enumeration, slow API timing, and the transition from web discovery to Java artifact analysis
## Why this matters for OSCP

This page matters because it turns a repeatable assessment task into a clear, reviewable habit for the OSCP exam.

## Escape case study: SMB evidence to MSSQL service-account coercion

Escape joins three information-gathering decisions. The full service scan identified a domain controller with SMB and MSSQL. Anonymous SMB then exposed a PDF, and the PDF supplied a SQL-authentication credential. The login was not sysadmin, so `xp_cmdshell` was denied. That negative result redirected the assessment to `xp_dirtree`, which made the SQL service account authenticate to an attacker-controlled UNC path.

```bash
smbclient -L "//$BoxIP" -N
smbclient "//$BoxIP/Public" -N \
  -c "lcd $BoxDir/loot/smb-public; recurse ON; prompt OFF; mget *"
pdftotext -layout "$BoxDir/loot/smb-public/SQL Server Procedures.pdf" \
  "$BoxDir/loot/sql-procedures.txt"
mssqlclient.py "$Domain/$PublicUser:$PublicPassword@$BoxIP" -port 1433
```
...
The key recon lesson is that one readable file can change the service branch, and one denied extended procedure does not imply that every MSSQL feature is unavailable. See [[OSCP/BOXES/WRITE UPS/Windows/Escape|Escape]], [[OSCP/COMMAND APPENDIX/SQL Injection & Databases|SQL Injection and Databases]], and [[OSCP/RUNBOOK V2/Windows - SMB Enum|Windows SMB Enum]].

## Related box coverage

- [[OSCP/BOXES/WRITE UPS/Windows/Escape|Escape]] -- domain-controller service triage, anonymous SMB PDF discovery, SQL authentication, and UNC hash coercion
