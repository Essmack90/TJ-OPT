...
- **[[8. Zenphoto|Zenphoto]]** (PG Practice, Linux, done ✅): Root of port 80 returned a blank placeholder page. Gobuster with `common.txt` found `/test/` — the entire attack surface. Without dir busting, nothing would have been found. Practical [[06. Information Gathering#6.5. Web Application Enumeration|web enumeration]] example. Technique: [[HTTP - Directory Brute]].
- **[[7. Nibbles|Nibbles]]** (PG Practice, Linux, done ✅): PostgreSQL 11.3 found on non-standard port 5437 via `-sV` service scan (nmap shows it as `pmip6-data` by port number alone). Default creds `postgres:postgres` on first try. Practical example of why service scanning matters over port-number-only triage.
