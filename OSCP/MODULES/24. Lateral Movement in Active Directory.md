...
## Vintage application: group-based RBCD to Kerberos WMI

Vintage used a group-shaped RBCD path. `DelegatedAdmins` was already in DC01's `AllowedToAct` relationship. The recovered `C.Neri_adm` account could add the pre-created, SPN-bearing `FS01$` computer account to that group. After verifying the member list and `tokenGroups`, a renewed `FS01$` TGT supported `getST.py` S4U2Self/S4U2Proxy impersonation of `L.Bianchi_adm`.

The resulting ccache was validated over SMB and consumed by `wmiexec.py`. The final scheduled task was only a reversible SYSTEM proof after the delegated session existed; it was not the RBCD mechanism itself. See [[OSCP/BOXES/WRITE UPS/AD/Vintage|Vintage]], [[OSCP/RUNBOOK V2/AD - Resource-Based Constrained Delegation|AD - Resource-Based Constrained Delegation]], and [[OSCP/RUNBOOK V2/AD - WinRM Foothold|AD - WinRM Foothold]].

## Search application: gMSA delegated reset to WMI proof

Search demonstrates a delegated lateral-movement route where the gMSA hash authenticates as a directory identity, BloodyAD changes the target user's password, and NetExec validates local administrator access before a WMI read command. This is distinct from pass-the-hash to an already-known administrator account.
...
See [[OSCP/BOXES/WRITE UPS/AD/Search|Search]], [[OSCP/RUNBOOK V2/AD - ForceChangePassword|AD - ForceChangePassword]], and [[OSCP/MODERN TOOLING/BloodyAD|BloodyAD]].
...
- [[OSCP/BOXES/WRITE UPS/AD/Fermion|Fermion]] -- authenticated SMB collection and pass-the-hash movement
- [[OSCP/BOXES/WRITE UPS/AD/RockyColt|RockyColt]] -- RBCD-based movement and ccache-authenticated WMI
- [[OSCP/BOXES/WRITE UPS/AD/Vintage|Vintage]] -- S4U2Proxy through a trusted group, Kerberos ccache validation, and WMI command execution
- [[OSCP/BOXES/WRITE UPS/AD/Blackfield|Blackfield]] -- Backup Operators/SeBackupPrivilege enabled protected-directory collection, offline NTDS extraction, and privileged WinRM validation
## Why this matters for OSCP

This page matters because it turns a repeatable assessment task into a clear, reviewable habit for the OSCP exam.

## Escape case study: certificate-derived pass-the-hash movement

Escape separated the identity transitions clearly. Ryan.Cooper used a vulnerable AD CS template to request a certificate with the Administrator UPN. Certipy authentication then returned the Administrator NT hash. The hash was validated over SMB and used with WinRM.
...
If WinRM is unavailable, an Impacket service shell is the fallback:
...
Pass-the-hash is not password cracking and does not require the cleartext password. The prerequisite is a valid NT hash for the correct account context. `netexec` validation is the decision point, and `whoami` plus the hostname prove the resulting session.

See [[OSCP/BOXES/WRITE UPS/Windows/Escape|Escape]], [[OSCP/RUNBOOK V2/AD - Pass the Hash|AD Pass the Hash]], and [[OSCP/MODULES/23. Attacking Active Directory Authentication|Module 23]].

## Related box coverage

- [[OSCP/BOXES/WRITE UPS/Windows/Escape|Escape]] -- AD CS impersonation to Administrator NT hash, SMB validation, and WinRM pass-the-hash
