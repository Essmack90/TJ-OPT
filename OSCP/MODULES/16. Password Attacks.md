...
**Cap case study:** [[OSCP/BOXES/WRITE UPS/Linux/Cap|Cap]] demonstrates the web-download-to-credential path: an IDOR exposed a PCAP, tshark isolated plaintext FTP authentication, and the recovered credential was validated once over SSH. Filter USER and PASS fields explicitly, store raw output privately, and remember that live anonymous-FTP denial is a separate result from captured authenticated traffic.

See [[OSCP/RUNBOOK V2/Linux - IDOR and PCAP Credential Recovery|Linux - IDOR and PCAP Credential Recovery]].

#### Tags: #CredentialHunting #cmdkey #runas #savecred #LaZagne #findstr #firefox_decrypt #Wireshark #Tshark #PCAP #CleartextCredentials #PasswordReuse #Snaffler #NetExec #Module16
...
- **[[OSCP/BOXES/WRITE UPS/Linux/Cap|Cap]]** (HTB, Linux, Easy): PCAP obtained via dashboard IDOR; tshark extracted plaintext FTP authentication; recovered credential was reused for SSH.
- **[[OSCP/BOXES/WRITE UPS/Linux/Management|Management]]** (HTB, Linux): an application-encrypted GLPI secret was recovered by reproducing the documented nonce/AD handling and tested as controlled SSH credential reuse.
- **[[OSCP/BOXES/WRITE UPS/Linux/Payday|Payday]]** (PG Practice, Linux): LFI-derived usernames were used to scope a controlled SSH password test.

#### Tags: #RelatedBoxes #Module16 #HTBBastion #HTBCascade #HTBForest #HTBArchetype #HTBQuerier #HTBBlue #HTBCap
...
## Search application: TGS, PFX, and one-password spray

Search used John for two different offline formats and NetExec for a controlled password-reuse check:
...
The box evidence distinguishes offline cracking from network validation and records the successful password-reuse hit separately. See [[OSCP/BOXES/WRITE UPS/AD/Search|Search]].

## Busqueda worked example: credentials crossing Git, Gitea, Docker, and SSH

Busqueda is a credential-reuse chain rather than a cracking exercise. The deployed Python application contained a Git remote with embedded HTTP authentication. The source username was not assumed to be the Linux account. The recovered value was recorded privately, then validated once against the local `svc` SSH account.
...
The next credential source was not a password file. A root-run Docker inspection wrapper exposed container environment values, including Gitea and MySQL configuration. This is a password-attack lesson because the environment values become authentication candidates, but the correct operation is controlled validation, not broad spraying.
...
sudo /usr/bin/python3 "$SudoScript" docker-inspect \
  "$DockerFormat" "$ContainerName" > "$BoxDir/loot/$ContainerName-env.json"
chmod 600 "$BoxDir/loot/$ContainerName-env.json"
```

Use the local Gitea API to validate the administrator candidate and keep the response private:
...
> A credential-bearing Git URL or Docker environment is sensitive evidence. Store the raw material privately, record its source and the service tested, and keep the value out of screenshots, logs intended for sharing, handoffs, and the vault.

> [!tip] Efficiency
> The source username, Linux username, Gitea administrator, and database account need not be the same identity. Preserve the source-to-service mapping. This prevents a valid password from being discarded simply because the first username guess fails.

See [[OSCP/BOXES/WRITE UPS/Linux/Busqueda|Busqueda]], [[OSCP/RUNBOOK V2/Linux - Credential Search|Linux - Credential Search]], and [[OSCP/RUNBOOK V2/Linux - Docker Enumeration|Linux - Docker Enumeration]].

#### Tags: #CredentialReuse #GitCredentials #DockerCredentials #Gitea #SSH #Module16
...
- [[OSCP/RUNBOOK V2/Linux - SSH Brute Force]]
- [[OSCP/RUNBOOK V2/Linux - Credential Search]]
- [[OSCP/RUNBOOK V2/Linux - Docker Enumeration]]
- [[OSCP/RUNBOOK V2/AD - AS-REP Roasting]]
- [[OSCP/RUNBOOK V2/AD - Credential Validation]]
- [[OSCP/RUNBOOK V2/Linux - IDOR and PCAP Credential Recovery]]
...
- [[OSCP/BOXES/WRITE UPS/Linux/Busqueda|Busqueda]] -- Git remote credential leakage, Docker environment secrets, Gitea validation, and SSH credential reuse
## Why this matters for OSCP

This page matters because it turns a repeatable assessment task into a clear, reviewable habit for the OSCP exam.

## Escape case study: Net-NTLMv2 capture, cracking, and credential reuse

Escape demonstrates a password attack that begins with service behavior rather than a login form. `xp_dirtree` forced the MSSQL service account to authenticate to Responder. The captured value was a Net-NTLMv2 challenge-response, so it was cracked offline with John. It was not an NT hash and could not be passed directly to SMB or WinRM.
...
The second credential source was a UTF-16LE SQL error-log backup. The log recorded a real username and an operator typo that exposed the intended password in the username field. The correct process was to preserve the raw file privately, decode it, map the username to the evidence, and validate the pair over WinRM. This is credential recovery and bounded reuse, not a reason to spray every account.
...
## Related box coverage

- [[OSCP/BOXES/WRITE UPS/Windows/Escape|Escape]] -- `xp_dirtree` Net-NTLMv2 capture, John cracking, UTF-16LE log typo leakage, and WinRM validation
