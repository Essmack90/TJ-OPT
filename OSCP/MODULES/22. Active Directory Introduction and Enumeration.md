...
## Vintage application: Kerberos-first enumeration and ACL graph reading

Vintage is a useful enumeration example because the supplied credential was valid, but NTLM was disabled. The service scan identified the domain and DC, Kerberos produced the usable TGT, and BloodHound was collected with `-k -no-pass` rather than a password-based NTLM path.

The important graph-reading sequence was:

```text
pre-created FS01$ -> ReadGMSAPassword -> gMSA01$
gMSA01$ -> GenericWrite/AddSelf -> ServiceManagers
ServiceManagers -> svc_sql
DelegatedAdmins -> DC01 AllowedToAct
```

The graph did not replace manual validation. LDAP confirmed the naming context, user/group memberships, disabled `svc_sql` state, and the `tokenGroups` refresh after `FS01$` was added to `DelegatedAdmins`. See [[OSCP/BOXES/WRITE UPS/AD/Vintage|Vintage]] and [[OSCP/RUNBOOK V2/AD - BloodHound|AD - BloodHound]].

## Search application: IIS-to-gMSA enumeration

Search adds a web-led AD enumeration path to this module. The IIS homepage exposed staff names and image files; the useful credential was handwritten inside one image. After LDAP/SMB validation, RedirectedFolders$ supplied a larger username set and the gMSA query returned both the managed account and PrincipalsAllowedToReadPassword.
...
See [[OSCP/BOXES/WRITE UPS/AD/Search|Search]], [[OSCP/RUNBOOK V2/AD - Web Enum|AD - Web Enum]], and [[OSCP/RUNBOOK V2/AD - PowerShell Web Access|AD - PowerShell Web Access]].
...
- [[OSCP/BOXES/WRITE UPS/AD/Vintage|Vintage]] -- Kerberos-only assumed breach, gMSA read, group ACLs, and manual validation of a group-based RBCD path
- [[OSCP/BOXES/WRITE UPS/AD/Blackfield|Blackfield]] -- anonymous SMB username discovery, AS-REP candidate identification, ForceChangePassword review, and protected-share enumeration
- [[OSCP/BOXES/WRITE UPS/AD/Active|Active]] -- anonymous SMB/GPP enumeration and recovery of a usable domain credential
- [[OSCP/BOXES/WRITE UPS/AD/Return|Return]] -- LDAP enumeration with passback and validation of delegated Windows service access
- [[OSCP/BOXES/WRITE UPS/AD/Fermion|Fermion]] -- authenticated share and host-role enumeration before offline credential extraction
- [[OSCP/BOXES/WRITE UPS/AD/RockyColt|RockyColt]] -- anonymous LDAP enumeration and delegation-rights discovery
## Why this matters for OSCP

This page matters because it turns a repeatable assessment task into a clear, reviewable habit for the OSCP exam.

## Escape case study: domain-controller triage and evidence-led enumeration

Escape is a useful enumeration example because the target was a domain controller, but the first useful directory clue came from anonymous SMB rather than LDAP. The complete port scan identified SMB, MSSQL, Kerberos, LDAP, WinRM, and AD Web Services. Share listing then exposed the `Public` share and a PDF containing SQL credentials.
...
The account and service context were kept separate. The PDF credential was tested against MSSQL SQL authentication. The later Net-NTLMv2 capture supplied a service-account password for WinRM, and the subsequent domain-user shell enabled certificate-template enumeration. A valid credential is not the end of enumeration. Re-run the relevant service and directory checks after each identity transition.

See [[OSCP/BOXES/WRITE UPS/Windows/Escape|Escape]], [[OSCP/RUNBOOK V2/AD - Service Scan|AD Service Scan]], and [[OSCP/RUNBOOK V2/Windows - SMB Enum|Windows SMB Enum]].

## Related box coverage

- [[OSCP/BOXES/WRITE UPS/Windows/Escape|Escape]] -- complete AD service triage, anonymous SMB document discovery, and identity-aware enumeration
