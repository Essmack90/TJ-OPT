...
### Grandpa case study: staged delivery and migration as a stability boundary

Grandpa is the framework exception worth understanding rather than memorising. The manual EDB-41738 adaptation was reviewed, byte-corrected, and tested with both reverse and bind payloads. The reverse attempt produced a delayed 500 with no callback, the bind attempt was reset, and repeated crashes triggered IIS Rapid Fail Protection.

The reason for the framework route was the IIS process model. The WebDAV overflow executes in `w3wp.exe`; a raw shell socket created there dies when the worker process dies. The IIS module's staged Meterpreter session was configured with `post/windows/manage/migrate`, so it moved into `notepad.exe` as soon as the stage arrived. Only then was the session suitable for local exploit triage.
...
The subsequent MS14-058 local module was selected from the validated legacy host context and returned SYSTEM. This case study demonstrates a narrow, technically justified framework use after a manual route has been exhausted and explained. See [[OSCP/BOXES/WRITE UPS/Windows/Grandpa|Grandpa]], [[Windows - Shell Received]], and [[Windows - Privilege Triage]].

#### Tags: #HTBGrandpa #IIS6 #CVE20177269 #StagedPayload #Meterpreter #Migration #MS14058 #Module21
...
- HTB **Granny**, token impersonation → getsystem path, dated but classic
- HTB **Grandpa**, IIS 6.0 WebDAV CVE-2017-7269, staged Meterpreter migration, and MS14-058 local escalation
...
- **Scope note:** the current verified Bastard write-up is a manual Drupalgeddon2 → SeImpersonate → JuicyPotato route; no Metasploit pivot was used
...
| HTB **Grandpa** | IIS WebDAV overflow, worker-process migration, and MS14-058 local escalation | `grandpa webdav migration` |
