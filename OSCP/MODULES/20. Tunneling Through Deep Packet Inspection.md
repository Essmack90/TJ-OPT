...
**Related:** [[19. Port Redirection and SSH Tunneling|Port Redirection and SSH Tunneling]] | [[Chisel]] | [[Dnscat2]] | [[Windows Methodology]] | [[Linux Methodology]]

---

## Why This Module Matters

Standard port-forwarding and SSH tunneling (Module 19) fail the moment a DPI device sits between Kali and the pivot. DPI inspects packet content, not just port numbers -- an SSH session on port 80 still looks like SSH to a DPI filter and gets dropped.

This module covers the two DPI-evasion channels:
- **HTTP tunneling (Chisel):** wraps everything in valid HTTP + SSH encryption. The DPI device sees ordinary web traffic. Used when only HTTP/HTTPS egress is permitted.
- **DNS tunneling (dnscat2):** encodes data in DNS queries and responses. Used when only UDP/53 (DNS) egress is permitted -- the last-resort channel.

> 🔑 **Key insight:** Protocol-aware firewalls inspect the first bytes of a stream to identify the protocol. Chisel's SSH-over-HTTP and dnscat2's DNS encoding both satisfy those protocol signatures, which is why they bypass DPI while raw reverse shells on non-standard ports don't.

> 🎓 **OSCP context:** These techniques appear in multi-segmented exam scenarios where one pivot host has strict egress rules. If standard SSH tunnels fail from a compromised host, try Chisel (HTTP) first, dnscat2 (DNS) second.
...
> 🔁 Similar to: [[19. Port Redirection and SSH Tunneling#SSH Remote Dynamic Port Forwarding|SSH remote dynamic forwarding]] but over HTTP instead of raw SSH.
...
> 🔁 Similar to: [[19. Port Redirection and SSH Tunneling|HTB PTF PT.4 Dnscat2]] for the setup commands. This section adds the port forwarding use case.
...
- HTB: [[OSCP/BOXES/MASTER BOX LIST|Dante]] (multi-hop SOCKS pivoting with Chisel)
- PG: most multi-host chains that require SOCKS (Chisel's forward SOCKS variant)

> 🔁 Similar to: [[19. Port Redirection and SSH Tunneling#19.4 Chisel|Module 19 Chisel forward SOCKS]] for the basic Chisel setup without the HTTP tunnel constraint and DPI context.

---

## Tags

#PortForwarding #SSHTunneling #Pivoting #Chisel #Dnscat2 #DPI #HTTPtunnel #DNStunnel #NetworkEvasion #Module20

---

*Module 20 fully complete as of 2026-08-23: theory, all four labs, Why This Module Matters added, solo enrichment pass done.*
## External Resources
