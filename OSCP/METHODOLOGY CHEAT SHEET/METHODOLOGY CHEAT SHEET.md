...
- Optimum cross-cutting pattern -- HFS version matching, reviewed command injection, clean Windows callback, Sherlock patch triage, and processor-aware MS16-098 selection are demonstrated in [[OSCP/BOXES/WRITE UPS/Windows/Optimum|Optimum]] and integrated into Windows Methodology
- Legacy cross-cutting pattern -- Windows XP SMBv1/RPC identification, inconsistent NSE validation, manual MS08-067 Python/Impacket porting, XP target-profile selection, and bind-shell proof are demonstrated in [[OSCP/BOXES/WRITE UPS/Windows/Legacy|Legacy]] and integrated into Windows Methodology
- Cap cross-cutting pattern -- bounded dashboard IDOR validation, private PCAP/tshark credential recovery, evidence-backed SSH reuse, and versioned Python CAP_SETUID escalation are demonstrated in [[OSCP/BOXES/WRITE UPS/Linux/Cap|Cap]] and integrated into Linux Methodology
- Management cross-cutting pattern -- application-secret recovery, controlled SSH credential reuse, and exact rdiff-backup sudo-boundary review are demonstrated in [[OSCP/BOXES/WRITE UPS/Linux/Management|Management]] and integrated into Linux Methodology
- Grandpa cross-cutting pattern -- IIS 6.0 WebDAV version/method enumeration, byte-preserving public-PoC adaptation, worker-process callback diagnosis, staged migration, and MS14-058 local escalation are demonstrated in [[OSCP/BOXES/WRITE UPS/Windows/Grandpa|Grandpa]] and integrated into Windows Methodology
