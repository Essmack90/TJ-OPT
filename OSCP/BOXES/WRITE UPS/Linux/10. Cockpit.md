...
> 📸 `shot nmap-services` (red boxes: Cockpit version `198–220` on 9090, title `blaze` on 80)
...
> 📸 `shot http-ferox` (red boxes: `/login.php` 200 line and `/logout.php` 302 line)
...
> 📸 `shot http-sqli-dashboard` (red boxes: `Welcome!! You are now logged in!` and the full credentials table — both usernames and base64 passwords)
...
> 📸 `shot cockpit-login` (red box: `james` in the top-right user indicator)
...
> 📸 `shot foothold` (red boxes: `james`, `uid=1000(james)`, `blaze`)
...
> 📸 `shot privesc-finding` (red box: the full sudo rule — specifically the trailing `*`)
...
> 📸 `shot root-shell` (red boxes: `/tmp/rootbash -p`, `rootbash-5.0#` prompt, `whoami` → `root`)
...
> 📸 `shot PROOF-Cockpit` (red boxes: `root`, `euid=0(root)`, `192.168.183.10`, flag value — all four in one frame)
...
- [ ] Screenshots confirmed in `$BoxDir/screenshots/`
