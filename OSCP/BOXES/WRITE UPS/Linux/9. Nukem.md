...
`shot nmap-services` (red boxes: port 80 WordPress, port 5000 Werkzeug)
...
`shot wordpress-plugin-version` (red box: `Stable tag: 4.2.2`)
...
`shot searchsploit` (red box: EDB-52371 row)
...
`shot upload-success` (red box: `SUCCESS` response)
...
`shot foothold` (red box: `uid=33(http)`)
...
`shot user-flag` (red box: flag value)
...
`shot privesc-finding` (red box: DB_PASSWORD line)
...
`shot privesc-finding` (red box: `/usr/bin/dosbox` in SUID list)
...
`shot privesc-exploit` (red box: `sudo -n id` showing `uid=0(root)`)
...
`shot PROOF` (red box: `uid=0(root)`, hostname `nukem`, IP `192.168.183.105`, flag value)
...
- [ ] Screenshots in `$BoxDir/screenshots/` (nmap-allports, nmap-services, wordpress-plugin-version, searchsploit, upload-success, foothold, user-flag, privesc-finding ×2, privesc-exploit, root-shell, PROOF)
- [ ] Loot: `flags.txt` (user + root), `creds.txt` (commander:CommanderKeenVorticons1990)
- [ ] Log copied to `OSCP/BOXES/BOX LOGS/Nukem.log`
- [ ] Stage notes: WordPress - Simple File List Upload (new), PrivEsc Linux - SUID (+DOSBox), Foothold - Public Exploit (+Nukem)
- [ ] Module notes: M08, M13, M18 (+Nukem)
- [ ] MASTER BOX LIST updated
- [ ] FAQ: plugin upload fields required, rename field names, mkfifo+nc failure, DOSBox SUID technique
