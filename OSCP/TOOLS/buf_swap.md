...
Replaces the `buf` block in any Python 2 Exploit-DB PoC and optionally patches the hardcoded target IP in `server_address`. Handles all common EDB buf formats (`""`, `b""`, one or two spaces, `+=` continuation lines).

**Location on disk:** `OSCP/TOOLS/buf_swap.py`
