...
- [[Web Applications (Breakdowns)|Web Applications]] -- WordPress XSS-to-admin chains, mass assignment, plugin metadata abuse; ffuf two-step filtering + -ac auto-calibrate + -mr regex match teardown; Pi-hole and IoT product fingerprinting with controlled credential validation; multipart XXE to source-driven pickle and Git-history credential analysis; Blocky REST user disclosure, exposed plugin JAR review, and slow-response timing triage.
...
- [[OSCP/BOXES/WRITE UPS/Linux/Poison|Poison route]] — explains the bounded repeated-Base64 decoding workflow and the SSH local forward to loopback VNC.
- [[Nostromo RCE (Breakdowns)|Nostromo RCE]] -- explains the reviewed Exploit-DB request, separate web and SSH-key cracking, and exact-argument journalctl pager escape.
- [[TLS Memory Disclosure and Tmux (Breakdowns)|TLS Memory Disclosure and Tmux]] -- explains Heartbleed evidence handling, old SSH key compatibility, and why a readable root-owned tmux socket is a direct session-hijack path.
...
- [[OSCP/BOXES/WRITE UPS/Linux/Valentine|Valentine]] -- demonstrates Heartbleed memory disclosure, private key handling, and tmux socket session access
...
- [[OSCP/BOXES/WRITE UPS/Linux/Mirai|Mirai]] -- demonstrates Pi-hole fingerprinting, IoT credential validation, and the decision to stop web enumeration when SSH is the stronger route
