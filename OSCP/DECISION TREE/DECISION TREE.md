...
- [[OSCP/BOXES/WRITE UPS/Windows/Grandpa|Grandpa route]] -- when IIS 6.0 WebDAV is identified, preserve and byte-check the PoC, diagnose worker-process callback failure, migrate before local triage, and select MS14-058 from host evidence.
...
- [[OSCP/BOXES/WRITE UPS/Linux/Management|Management route]] -- when a web foothold exposes application secrets, validate the credential pivot, then inspect exact sudo arguments and backup-tool restrictions.
...
- [[Buffer Overflow & Memory Corruption (Decision Tree)|Buffer Overflow & Memory Corruption]] — rotated EIP values, target crashes mid-exploit, missing/wrong return addresses, multiple candidate exploits, Covfefe's source-derived adjacent-string overwrite, Unicode overflow blob integrity, worker-process shell lifetime, and Rapid Fail Protection
...
## Cap cross-cutting pattern

Cap joins three decision points that are easy to leave disconnected: bounded IDOR validation, protocol-aware analysis of a downloaded PCAP, and file-capability triage after SSH credential reuse. The complete route is documented in [[OSCP/BOXES/WRITE UPS/Linux/Cap|Cap]], [[OSCP/RUNBOOK V2/Linux - IDOR and PCAP Credential Recovery|Linux - IDOR and PCAP Credential Recovery]], and [[OSCP/RUNBOOK V2/Linux - File Capabilities|Linux - File Capabilities]].
...
- **Source review can turn a low-impact upload into a privilege chain.** Download exposed backups, map the multipart field and filename transformation, confirm a harmless webshell identity, then trace any scheduled worker and privileged configuration consumer. See [[OSCP/BOXES/WRITE UPS/Linux/Networked|Networked]].
...
- [[OSCP/BOXES/WRITE UPS/Linux/DevOops|DevOops]] -- demonstrates multipart XML XXE, source-driven pickle proof, and Git-history credential review
- [[OSCP/BOXES/WRITE UPS/Linux/Mirai|Mirai]] -- demonstrates IoT product fingerprinting, `/admin/` discovery, controlled credential validation, and direct sudo escalation
