...
→ Straight port-scan-and-exploit has nothing to reach. Switch to a client-side vector instead: recon the target org first ([[12. Client-Side Attacks#12.1. Target Reconnaissance|12.1]], document metadata + Canarytoken fingerprinting), then pick Office macros ([[12. Client-Side Attacks#12.2. Exploiting Microsoft Office|12.2]]) or Windows library files ([[12. Client-Side Attacks#12.3. Abusing Windows Library Files|12.3]]) based on what the recon told you about the target's OS/software.
...
→ See [[12. Client-Side Attacks#🔁 Lab 1 Rebuild (fresh instance, after prior OFFICE VM corruption)|Lab 1 Rebuild troubleshooting]].
...
→ See [[12. Client-Side Attacks#Step 4: Build the `.lnk` shortcut payload (the actual reverse-shell trigger)|12.3.1, Step 4]].
