...
> Quick symptom-to-technique lookup: [[DECISION TREE#Web Applications]]
...
- `INTO OUTFILE` (MySQL) and `xp_cmdshell` (MSSQL) are the two paths from SQLi to OS command execution, sqlmap's `--os-shell` automates the MySQL one
