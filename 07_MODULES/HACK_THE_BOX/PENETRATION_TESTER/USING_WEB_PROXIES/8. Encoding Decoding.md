...
In ZAP, we can use the `Encoder/Decoder/Hash` tool, which will automatically decode strings using various decoders in the `Decode` tab: ![Text encoding/decoding interface showing Base64 encoded string and its decoded JSON output.](https://academy.hackthebox.com/storage/modules/110/zap_b64_decode.png)
