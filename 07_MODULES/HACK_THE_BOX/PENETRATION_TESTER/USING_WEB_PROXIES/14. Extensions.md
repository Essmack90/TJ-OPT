...
In this tab, we can see the different available add-ons for ZAP. Some add-ons may be in their `Release` build, meaning that they should be stable to be used, while others are in their `Beta/Alpha` builds, which means that they may experience some issues in their use. Let's try installing the FuzzDB Files and FuzzDB Offensive add-ons, which add new wordlists to be used in ZAP's fuzzer:  ![Marketplace tab showing add-ons. Selected: FuzzDB Files and FuzzDB Offensive, both in Release status. Options to install or get more info.](https://academy.hackthebox.com/storage/modules/110/zap_fuzzdb_install.png)
