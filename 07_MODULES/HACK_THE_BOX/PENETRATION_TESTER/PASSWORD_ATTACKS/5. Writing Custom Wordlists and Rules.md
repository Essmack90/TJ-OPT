...
EssMack90@htb[/htb]`$ cat password.list  password`
...
EssMack90@htb[/htb]`$ cat custom.rule  : c so0 c so0 sa@ c sa@ c sa@ so0 $! $! c $! so0 $! sa@ $! c so0 $! c sa@ $! so0 sa@ $! c so0 sa@`
...
EssMack90@htb[/htb]`$ hashcat --force password.list -r custom.rule --stdout | sort -u > mut_password.list`

In this case, the single input word will produce fifteen mutated variants.

EssMack90@htb[/htb]`$ cat mut_password.list  password Password passw0rd Passw0rd p@ssword P@ssword P@ssw0rd password! Password! passw0rd! p@ssword! Passw0rd! P@ssword! p@ssw0rd! P@ssw0rd!`
...
EssMack90@htb[/htb]`$ cewl https://www.inlanefreight.com -d 4 -m 6 --lowercase -w inlane.wordlist $ wc -l inlane.wordlist  326`
