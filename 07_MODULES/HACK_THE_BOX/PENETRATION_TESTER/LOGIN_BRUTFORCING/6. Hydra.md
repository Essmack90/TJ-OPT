...
EssMack90@htb[/htb]`$ hydra -h`

If Hydra is not installed or you are using a different Linux distribution, you can install it from the package repository:

EssMack90@htb[/htb]`$ sudo apt-get -y update $ sudo apt-get -y install hydra` 

## Basic Usage

Hydra's basic syntax is:

EssMack90@htb[/htb]`$ hydra [login_options] [password_options] [attack_options] [service_options]`
...
EssMack90@htb[/htb]`$ hydra -L usernames.txt -P passwords.txt www.example.com http-get`
...
EssMack90@htb[/htb]`$ hydra -l root -p toor -M targets.txt ssh`
...
EssMack90@htb[/htb]`$ hydra -L usernames.txt -P passwords.txt -s 2121 -V ftp.example.com ftp`
...
EssMack90@htb[/htb]`$ hydra -l admin -P passwords.txt www.example.com http-post-form "/login:user=^USER^&pass=^PASS^:S=302"`
...
EssMack90@htb[/htb]`$ hydra -l administrator -x 6:8:abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 192.168.1.100 rdp`
