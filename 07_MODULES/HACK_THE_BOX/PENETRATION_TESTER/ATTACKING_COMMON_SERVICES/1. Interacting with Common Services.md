...
EssMack90@htb[/htb]`$ sudo mkdir /mnt/Finance $ sudo mount -t cifs -o username=plaintext,password=Password123,domain=. //192.168.220.129/Finance /mnt/Finance`

As an alternative, we can use a credential file.

EssMack90@htb[/htb]`$ mount -t cifs //192.168.220.129/Finance /mnt/Finance -o credentials=/path/credentialfile`
...
EssMack90@htb[/htb]`$ find /mnt/Finance/ -name *cred*  /mnt/Finance/Contracts/private/credentials.txt`

Next, let's find files that contain the string `cred`:

EssMack90@htb[/htb]`$ grep -rn /mnt/Finance/ -ie cred  /mnt/Finance/Contracts/private/credentials.txt:1:admin:SecureCredentials! /mnt/Finance/Contracts/private/secret.txt:1:file with all credentials`
...
EssMack90@htb[/htb]`$ sudo apt-get install evolution ...SNIP...`
...
EssMack90@htb[/htb]`$ sqsh -S 10.129.20.13 -U username -P Password123`
...
EssMack90@htb[/htb]`$ mysql -u username -pPassword123 -h 10.129.20.13`
...
EssMack90@htb[/htb]`$ sudo dpkg -i dbeaver-<version>.deb`

To start the application use:

#### Run dbeaver

EssMack90@htb[/htb]`$ dbeaver &`
