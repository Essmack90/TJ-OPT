...
We will leverage Scrapy and a custom spider tailored for reconnaissance on `inlanefreight.com`. If you are interested in more information on crawling/spidering techniques, refer to the "[Using Web Proxies](https://enterprise.hackthebox.com/content-library?s=Using%20Web%20Proxies)" module, as it forms part of CWES as well.
...
EssMack90@htb[/htb]`$ pip3 install scrapy`
...
EssMack90@htb[/htb]`$ wget -O ReconSpider.zip https://academy.hackthebox.com/storage/modules/144/ReconSpider.v1.2.zip $ unzip ReconSpider.zip` 

With the files extracted, you can run `ReconSpider.py` using the following command:

EssMack90@htb[/htb]`$ python3 ReconSpider.py http://inlanefreight.com`
