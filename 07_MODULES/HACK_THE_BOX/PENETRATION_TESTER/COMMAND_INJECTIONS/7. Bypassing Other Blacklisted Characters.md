...
EssMack90@htb[/htb]`$ echo ${PATH}  /usr/local/bin:/usr/bin:/bin:/usr/games`

So, if we start at the `0` character, and only take a string of length `1`, we will end up with only the `/` character, which we can use in our payload:

EssMack90@htb[/htb]`$ echo ${PATH:0:1}  /`
...
EssMack90@htb[/htb]`$ echo ${LS_COLORS:10:1}  ;`
...
EssMack90@htb[/htb]`$ man ascii     # \ is on 92, before it is [ on 91 $ echo $(tr '!-}' '"-~'<<<[)  \`
