...
EssMack90@htb[/htb]`$ git clone https://github.com/tmux-plugins/tpm ~/.tmux/plugins/tpm`

Next, create a `.tmux.conf` file in the home directory.

EssMack90@htb[/htb]`$ touch .tmux.conf`

The config file should have the following contents:

EssMack90@htb[/htb]`$ cat .tmux.conf   # List of plugins  set -g @plugin 'tmux-plugins/tpm' set -g @plugin 'tmux-plugins/tmux-sensible' set -g @plugin 'tmux-plugins/tmux-logging'  # Initialize TMUX plugin manager (keep at bottom) run '~/.tmux/plugins/tpm/tpm'`
...
EssMack90@htb[/htb]`$ tmux source ~/.tmux.conf` 
...
EssMack90@htb[/htb]`$ mkdir -p ACME-IPT/{Admin,Deliverables,Evidence/{Findings,Scans/{Vuln,Service,Web,'AD Enumeration'},Notes,OSINT,Wireless,'Logging output','Misc Files'},Retest}`

EssMack90@htb[/htb]`$ tree ACME-IPT/  ACME-IPT/ ├── Admin ├── Deliverables ├── Evidence │   ├── Findings │   ├── Logging output │   ├── Misc Files │   ├── Notes │   ├── OSINT │   ├── Scans │   │   ├── AD Enumeration │   │   ├── Service │   │   ├── Vuln │   │   └── Web │   └── Wireless └── Retest`
...
EssMack90@htb[/htb]`$ xfreerdp /v:10.129.203.82 /u:htb-student /p:HTB_@cademy_stdnt!`
