...
EssMack90@htb[/htb]`$ sudo apt install vsftpd` 
...
EssMack90@htb[/htb]`$ cat /etc/vsftpd.conf | grep -v "#"`
...
EssMack90@htb[/htb]`$ cat /etc/ftpusers  guest john kevin`
...
EssMack90@htb[/htb]`$ ls | grep Notes.txt  'Important Notes.txt'`
...
EssMack90@htb[/htb]`$ tree .  . └── 10.129.14.136     ├── Calendar.pptx     ├── Clients     │   └── Inlanefreight     │       ├── appointments.xlsx     │       ├── contract.docx     │       ├── meetings.txt     │       └── proposal.pptx     ├── Documents     │   ├── appointments-template.xlsx     │   ├── contract-template.docx     │   └── contract-template.pdf     ├── Employees     └── Important Notes.txt  5 directories, 9 files`
...
EssMack90@htb[/htb]`$ touch testupload.txt`
...
EssMack90@htb[/htb]`$ sudo nmap --script-updatedb  Starting Nmap 7.80 ( https://nmap.org ) at 2021-09-19 13:49 CEST NSE: Updating rule database. NSE: Script Database updated successfully. Nmap done: 0 IP addresses (0 hosts up) scanned in 0.28 seconds`
